...
//                    }
            ...
            override fun deleteProgram(programId: String): Observable<Program> {
                return Observable.create<Program> { emitter ->
                    Thread.sleep(5000)
                    emitter.onNext(fakePrograms()[0])
                    emitter.onComplete()
                }
            }
...
//            .build().create(ServerApi::class.java)
    ...
    private fun fakePrograms() : List<Program> {
        ...
        programs.add(Program.Builder(null).id("1").name("Aipa Cascade Max").active(true).sensorId("sensor_2").coolingRelayIndex(1).maxTemp(19.4).minTemp(18.8).build())
